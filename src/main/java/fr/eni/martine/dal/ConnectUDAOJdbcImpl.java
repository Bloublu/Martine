...
public class ConnectUDAOJdbcImpl implements ConnectUDAO {
	...
	public User ConnectionUser(String identifiant, String Mdp) throws DalException {
		...
		try (Connection cnx = ConnectionProvider.getPoolConnexion()) {
			...
			while (rs.next()) {
				CUser = new User(rs.getInt("no_utilisateur"), rs.getString("pseudo"), rs.getString("nom"),
	...
	public List<Enchere> selectAll() throws DalException {
		...
		try (Connection cnx = ConnectionProvider.getPoolConnexion()) {
			...
			while (rs.next()) {
				enchere = new Enchere(rs.getTime("date_enchere").toLocalTime(), rs.getInt("montant_enchere"));
				article = new Article(rs.getString("nom_article"), rs.getString("description"),
						rs.getTime("date_debut_enchere").toLocalTime(), rs.getTime("date_fin_enchere").toLocalTime(),
						rs.getInt("prix_initial"), rs.getInt("prixvente"), rs.getInt("no_categorie"),
						rs.getString("etat_vente"), rs.getString("image"));
				utilisateur = new User(rs.getInt("no_utilisateur"), rs.getString("pseudo"), rs.getString("nom"),
						rs.getString("prenom"), rs.getString("email"), rs.getString("telephone"), rs.getString("rue"),
						rs.getString("code_postal"), rs.getString("ville"), rs.getString("mot_de_passe"),
						rs.getInt("credit"), rs.getBoolean("administrateur"));
